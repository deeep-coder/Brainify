...
import { userModel,contentModel,linkModel } from "../models/userdb.js";
import { JWT_SECRET } from "../config.js";
import {userSignupSchema,userSigninSchema} from '../schema/user.js';

export const userSignup = async (req: Request, res: Response) => {
    try {
        const parsed = userSignupSchema.safeParse(req.body); // Checks if req.body matches schema
        if (!parsed.success) return res.status(400).json({ message: "Invalid data" });

        const { username, email, password } = parsed.data; //Only runs if validation passed , Extracts clean & validated data

        const hashed = await bcrypt.hash(password, 10); // convert plain password into hashed password using salt value as 10
